...
download_type = st.selectbox("Select type", ("Video - mp4", "Audio - original"))
...
def download_audio(yt, output_dir):
        ...
        try:
            if "playlist" in link.lower():
                ...
                for idx, video_url in enumerate(p.video_urls, start=1):
                    ...
                    if download_type == "Audio - original":
                        success, info = download_audio(yt, DOWNLOAD_DIR)
                        if success:
                            st.success(f"✅ Downloaded Audio: {yt.title}")
                        else:
                            st.warning(f"⚠️ {info}")
                    else:  # Video
...
st.caption("Audio downloads are saved as webm/mp4 (playable on most devices). For playlists, this may take time.")
